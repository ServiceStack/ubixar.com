    ...
    private void LoadJobQueue()
    {
        ...
        var commandDurations = db.Dictionary<string, int>(
            db.From<JobSummary>()
                ...
                .Select(x => new {
                    Command = Sql.Custom($"CASE WHEN {columns.Worker} is null THEN {columns.Command} ELSE {columns.Command} || '.' || {columns.Worker} END"), 
        ...
        while (updates.TryDequeue(out var update))
        {
            try
            {
                ...
                if (update.Log != null)
                {
                    ...
                    fieldUpdates.Add($"{columns.Logs} = CASE WHEN {columns.Logs} IS NOT NULL THEN {columns.Logs} || {newLine} || @log ELSE @log END");
